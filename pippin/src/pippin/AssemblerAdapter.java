	...
	public void assemble(){
		...
		if (source==null || !source.exists()){
			...
			if(machine.getDirectoryManager().getExecutableDir() == null 
					|| machine.getDirectoryManager().getExecutableDir().equals(machine.getDirectoryManager().getEclipseDir())){
			...
			if (outputExe == null){
				JOptionPane.showMessageDialog(machine.getFrame(), "The output file was not selected or could not be created.\n" +
						"Cannot assemble the program");
			}
			else{
				...
				if (response.equals("goodProgram")){
					...
					JOptionPane.showMessageDialog(machine.getFrame(), "The selected file has problems. The assembler error is:\n" + response);
				}
			}
		}
	}
