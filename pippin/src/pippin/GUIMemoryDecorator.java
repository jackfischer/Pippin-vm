...
import java.awt.Color; import java.awt.Rectangle; import java.util.Observable; import java.util.Observer;

import javax.swing.JLabel; import javax.swing.JTextField;

public class GUIMemoryDecorator implements MemoryInterface, Observer { public static final Color HIGHLIGHT_COLOR = Color.YELLOW; public static int FIELD_WIDTH = 10;
...
public void setData(int index, int value) throws DataAccessException {
    ...
    if(currentDataHighlight >= 0 && currentDataHighlight < dataComponents.length) {
        dataComponents[currentDataHighlight].fillCells();
        dataComponents[currentDataHighlight].setColor(HIGHLIGHT_COLOR);
        dataHighlight = dataComponents[currentDataHighlight].rowIndex.getBounds();
    }
}
...
private class DataGUI {
    ...
    public DataGUI(int loc) {
        ...
        rowValue = new JTextField(FIELD_WIDTH);
        rowHex = new JTextField(FIELD_WIDTH);
...
private class CodeGUI {     
    ...
    public CodeGUI(int loc) {
        location = loc;
        rowIndex = new JLabel(loc+": ", JLabel.RIGHT);
        rowInstr = new JTextField(FIELD_WIDTH);
        rowHex = new JTextField(FIELD_WIDTH);
        rowIndex.setOpaque(true);
    }
    ...
    if(indirect) inString += "#";
    rowInstr.setText(inString + " " + Integer.toHexString(arg).toUpperCase());
